...
void zcache_resize(void)
{
	...
	do {
		...
		wait_event_interruptible_timeout(zcache_wait, ((nr_free_pages()-nr_zcache_free_pages >= COLDMEM_MAP_MAX_PAGES*2) && (nr_free_pages() > last_watermark + COLDMEM_MAP_MAX_PAGES)) || kthread_should_stop(), ZCACHE_TIMEOUT);
		...
		free_pages = nr_free_pages() - nr_zcache_free_pages;
		...
		if ((last_watermark > 500000) && (free_pages > 2000000))	{
			compact_strike++;
			if (compact_strike == 3)	{	
				compact_memory();
		...
		printk ("zcache_resize: current_pool %d free_pages:%lu grabbed_pages: %d last_watermark %lu free mem: %lu used mem:%lu..\n", 
						current_pool, free_pages, grabbed_pages, last_watermark, zcache_free_memory_count(), zcache_used_memory_count());	
		msleep(13000);
	}	while (1);
...
int zcache_release_unused_pool(void)
{
    ...
    for (i=50; i< MAX_PASR_POOLS_PER_DIMM; i++)    { // CHANGE it back to 0
        pool_busy = 0;
        for (j=0; j< COLDMEM_MAP_MAX_PAGES; j++)  {
