...
/*** From source: test-models/bakery.8.pm ***/
...
#define Index(x, y)	Boundcheck(x, y, 0, 0, 0)
	...
	case 4:	/* P_4 */
		((P4 *)pptr(h))->_t = 4;
		((P4 *)pptr(h))->_p = 6; reached4[6]=1;
		/* params: */
		/* locals: */
		((P4 *)pptr(h))->j = 0;
		((P4 *)pptr(h))->max = 0;
#ifdef VAR_RANGES
		logval("P_4:j", ((P4 *)pptr(h))->j);
		logval("P_4:max", ((P4 *)pptr(h))->max);
#endif
#ifdef HAS_CODE
		locinit4(h);
#endif
	...
	case 3:	/* P_3 */
		((P3 *)pptr(h))->_t = 3;
		((P3 *)pptr(h))->_p = 6; reached3[6]=1;
		/* params: */
		/* locals: */
		((P3 *)pptr(h))->j = 0;
		((P3 *)pptr(h))->max = 0;
#ifdef VAR_RANGES
		logval("P_3:j", ((P3 *)pptr(h))->j);
		logval("P_3:max", ((P3 *)pptr(h))->max);
#endif
#ifdef HAS_CODE
		locinit3(h);
#endif
	...
	case 2:	/* P_2 */
		...
		((P2 *)pptr(h))->max = 0;
#ifdef VAR_RANGES
		logval("P_2:j", ((P2 *)pptr(h))->j);
		logval("P_2:max", ((P2 *)pptr(h))->max);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* P_1 */
		((P1 *)pptr(h))->_t = 1;
		((P1 *)pptr(h))->_p = 6; reached1[6]=1;
		/* params: */
		/* locals: */
		((P1 *)pptr(h))->j = 0;
		((P1 *)pptr(h))->max = 0;
#ifdef VAR_RANGES
		logval("P_1:j", ((P1 *)pptr(h))->j);
		logval("P_1:max", ((P1 *)pptr(h))->max);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* P_0 */
		((P0 *)pptr(h))->_t = 0;
		((P0 *)pptr(h))->_p = 6; reached0[6]=1;
		/* params: */
		/* locals: */
		((P0 *)pptr(h))->j = 0;
		((P0 *)pptr(h))->max = 0;
#ifdef VAR_RANGES
		logval("P_0:j", ((P0 *)pptr(h))->j);
		logval("P_0:max", ((P0 *)pptr(h))->max);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(void)
{
	...
	{	int l_in;
		for (l_in = 0; l_in < 5; l_in++)
		{
			logval("choosing[l_in]", now.choosing[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 5; l_in++)
		{
			logval("number[l_in]", now.number[l_in]);
		}
	}
#endif
...
{	/* int i; */
	...
	{	int l_in;
		for (l_in = 0; l_in < 5; l_in++)
		{
			printf("	byte   choosing[%d]:	%d\n", l_in, now.choosing[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 5; l_in++)
		{
			printf("	byte   number[%d]:	%d\n", l_in, now.number[l_in]);
		}
	}
}
void
...
{	/* int i; */
	switch(tp) {
	case 4:
		printf("local vars proc %d (P_4):\n", pid);
	printf("	byte   j:	%d\n", ((P4 *)pptr(pid))->j);
	printf("	byte   max:	%d\n", ((P4 *)pptr(pid))->max);
		break;
	case 3:
		printf("local vars proc %d (P_3):\n", pid);
	printf("	byte   j:	%d\n", ((P3 *)pptr(pid))->j);
	printf("	byte   max:	%d\n", ((P3 *)pptr(pid))->max);
		break;
	case 2:
		printf("local vars proc %d (P_2):\n", pid);
	printf("	byte   j:	%d\n", ((P2 *)pptr(pid))->j);
	printf("	byte   max:	%d\n", ((P2 *)pptr(pid))->max);
		break;
	case 1:
		printf("local vars proc %d (P_1):\n", pid);
	printf("	byte   j:	%d\n", ((P1 *)pptr(pid))->j);
	printf("	byte   max:	%d\n", ((P1 *)pptr(pid))->max);
		break;
	case 0:
		printf("local vars proc %d (P_0):\n", pid);
	printf("	byte   j:	%d\n", ((P0 *)pptr(pid))->j);
	printf("	byte   max:	%d\n", ((P0 *)pptr(pid))->max);
		break;
	}
