...
static int (*mcache_free_one_pool) (void);
...
void register_mcache_pool_free_fn(void (*odft_fn)(void *) )
{
  mcache_pool_free_fn = odft_fn;
}
EXPORT_SYMBOL(register_mcache_pool_free_fn);
...
void register_mcache_free_pool_fn(int (*odft_fn)(void))
{
	mcache_free_one_pool = odft_fn;
	printk ("free function registered...\n");
}

EXPORT_SYMBOL(register_mcache_free_pool_fn);

int zcache_free_one_pool(void)
{
...
#ifdef ZCACHE_SEPARATE_PAGE
	zh->zpage = zcache_get_free_page(&poolid);
	to = zh->zpage;
	kmap_atomic(to);
#endif
...
static int zbud_hash_compare(struct page *page, struct zbud_hdr *zh)
{
	struct zbud_page *zbpg;
	unsigned budnum = zbud_budnum(zh);
	unsigned int out_len = PAGE_SIZE;
	char *to_va;
	//unsigned char newdigest[MCACHE_DIGEST_SIZE];
	u32 newdigest = 0;
	unsigned size;
	int ret = 0;
	...
	*/
	...
	/*
	ret = zcache_comp_op(ZCACHE_COMPOP_CHECKSUM, to_va, PAGE_SIZE,
				NULL, &out_len, newdigest);

	BUG_ON(ret);
	*/
//#ifdef ZCACHE_DEBUG_PRINT
	if (pagecount %50 ==0)	{
		printk ("stored hash %llx . new hash %llx. \n", zh->digest, newdigest);
	/*
		printk ("stored hash. %llX %llX %llX %llX \n", (unsigned long) zh->digest[0], (unsigned long) zh->digest[1],
						(unsigned long) zh->digest[2], (unsigned long)zh->digest[3]  ); 		
		printk ("new hash %llX %llX %llX %llX \n", (unsigned long)newdigest[0], (unsigned long) newdigest[1], (unsigned long) newdigest[2],
						(unsigned long) newdigest[3]);
	*/
		printk ("+++++++++++++++++++++++++++++++++\n");	
...
//#endif
	if (newdigest != zh->digest)
//	if (!memcmp(newdigest, zh->digest, MCACHE_DIGEST_SIZE*8)) 
	{
					...
					struct tmem_oid *oid, uint32_t index)
{
	...
	mb();
	memcpy(to_va, zh->zpage, PAGE_SIZE);
	//kunmap_atomic(to_va);
	BUG_ON(data == NULL);
#else
...
/*
	if (zbud_hash_compare((struct page *)(data), pampd) != 0) {
		printk("hash comparision failed..\n");
		corrupt_pampd = 1;
	}
*/
