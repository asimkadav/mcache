...
static int zbud_hash_compare(struct page *page, struct zbud_hdr *zh)
{
	...
	size = zh->size;
	from_va = zbud_data(zh, size);
	ret = zcache_comp_op(ZCACHE_COMPOP_CHECKSUM, from_va, size,
				to_va, &out_len, newdigest);
	BUG_ON(ret);

	pagecount++;
	if (pagecount %50 ==0)	{
		printk ("stored hash. %l %l %l %l\n",zh->digest[0],zh->digest[1]
						,zh->digest[2], zh->digest[3]  ); 		
		printk ("new hash %l %l %l %l\n", newdigest[0], newdigest[1], newdigest[2],
						newdigest[3]);
		printk ("+++++++++++++++++++++++++++++++++\n");	
	...
	if (!memcmp(newdigest, zh->digest,
                    MCACHE_DIGEST_SIZE*8)) {
	   return -1; 	
	}
